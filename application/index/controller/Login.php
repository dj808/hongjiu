...
class Login extends Base
{
    ...
    public function index()
    {
		...
		$inviteId=$this->request->get('user_id');
		if($inviteId==NUll){
			$inviteId=23;
		}
        //微信授权获取基本信息
        if ($this->request->param('code')==''){//没有code，去微信接口获取code码	     
            $this->getcodess($inviteId);
			
        } else {//获取code后跳转回来到这里了
            $code=$this->request->param('code');
			
			$userId=$this->request->get('inviteId');
			...
			$res=Db::name('join_num')->field('phone')->where('open_id',session('useropenid'))->find();
			  if($res['phone']){
    ...
    //授权反馈
    public function getcodess($inviteId)
    {
        //获取微信配置参数
        $wxConfig = Config::get('wechat');
        $appId=$wxConfig['AppId'];
        file_put_contents("log3233.log",1);
        $url = "http://www.loungwangtouzi.com/index/login/index?inviteId=$inviteId";
       // $url = urlencode($url);
